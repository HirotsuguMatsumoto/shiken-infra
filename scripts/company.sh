...
(cd ./company-project && docker-compose -f docker-compose.yml down --rmi local -v && cd ..) || true

# Run this from the root of the project
rm -rf ./company-project

cookiecutter --no-input -f ./ project_slug="company-project" project_name="Company project"
...
# Run backend tests
docker-compose exec -T postgres dropdb -U postgres --if-exists app
docker-compose exec -T postgres createdb -U postgres app
docker-compose exec -T postgres createdb -U postgres apptest

# Create Company Table (マイグレーションが走らないので手動で作成)
# 下記のサイトのようにディレクトリを/docker-entrypoint-initdb.dにマウントさせて
# そのディレクトリにsqlファイルを配置しても良い.
# thx:https://zenn.dev/re24_1986/articles/b76c3fd8f76a
docker-compose exec -T postgres psql -U postgres app < ./backend/companys_table.sql
